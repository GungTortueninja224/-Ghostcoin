...
impl Miner {
    ...
    pub fn mine_block(&mut self, _chain: &SharedChain) -> MinedBlock {
        ...
        let data = format!(
            ...
            reward, fees_collected,
            selected_txs.len(), index,
        );

        log_mining(&format!("Mining bloc {} | {} TX | {} GHST reward | {} GHST fees",
            index, selected_txs.len(), reward, fees_collected));
        ...
        state.add_block(&hash, total_reward, fees_collected, selected_txs.len() as u64);
        self.total_mined += total_reward;

        log_mining(&format!("Bloc #{} confirmé | Supply: {} GHST",
