...
impl SwapEngine {
    ...
    pub fn execute_swap(&mut self, from: &str, to: &str, amount: f64) -> Option<SwapResult> {
        // Vérifie solde
        ...
        let received = self.prices.convert_from_usd(net_usd, to);
...
impl SwapResult {
    pub fn show(&self) {
        ...
        println!("║  Frais      : {:<43} ║", format!("${:.4}", self.fee_usd));
        println!(
            "║  TX ID      : {:<43} ║",
